    ...
    def __init__(self, sentence_size, categories, vocabulary_size, embedding_size, filters, filter_size):
        ...
        for i, filter in enumerate(filters):
            with tf.name_scope("convolution-maxpool-%s" % filter):
                ...
                #activation function
                h_conv = tf.nn.relu(tf.nn.bias_add(conv, b), name="relu")
                #maxpool
        ...
        num_filters_total = filter_size * len(filters)
        self.h_pool = tf.concat(3, total_maxpools)
        ...
        with tf.name_scope("output"):
            W = tf.get_variable("W", shape=[num_filters_total, categories], initializer=tf.contrib.layers.xavier_initializer())
